...
def generate_golds(n_golds, prefix='gold_object_labels_quite_long_'):
    return _generate_elements(n_golds, prefix)
...
def generate_core_items(n_workers, n_objects, n_golds, n_labels):
    return generate_workers(n_workers), generate_objects(n_objects), \
        generate_golds(n_golds), generate_labels(n_labels)


def genereate_votes(n_votes, workers, objects, golds, labels):
    objects = objects + golds
...
def generate_data(n_votes, n_labels=2, n_objects=None,
    ...
    workers, objects, golds, labels = generate_core_items(n_workers,
        n_objects, n_golds, n_labels)
